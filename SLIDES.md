---
title: "UXN"
sub_title: A Computer at Human Scale
author: Boen Shao
theme:
  name: gruvbox-dark
---
...
* I build Vision AI Systems at work
* My blog -> https://unsungmoments.bearblog.dev
* We are hiring -> https://www.cake.me/companies/berryai/jobs
...
* JVM (Java), CIL (.NET), BEAM (Erlang)

<!-- pause -->

# UXN is Neither

* UXN doesn't emulate existing hardware; it has its own spec
* UXN is designed to be programmed in its own assembly
* UXN is for FUN!
...
* The spec is minimal and can be implemented in an afternoon, making it a great subject for learning new languages and platforms.
* **Uxntal** is a "concatenative" flavoured assembly, quite fun to program in. (Any Forth fan here?)
...
CC BY-NC-SA 4.0, https://wiki.xxiivv.com/media/generic/uxn.crew.png
...
* 16-bit, 64kB of addressable memory, big-endian
...
* `.` - return stack
* `P` - program counter, `P+:a` - jump by `a` in byte mode, set to `a` in short mode
...
* On soft-reboot, the zero-page (0x00-0xff) is preserved.
...
A Quote from Hackernews
===

# https://news.ycombinator.com/item?id=41806206

> On my laptop, the Left text editor running in Uxn (with SDL!) still uses less energy than Emacs does, despite the interpretation overhead.
> And you can run it on a GameBoy Advance, so it's definitely a step in the right direction if you're looking for a frugal write-once/run-anywhere platform.
...
![image:width:30%](images/uxn.team.png)
CC BY-NC-SA 4.0, https://wiki.xxiivv.com/media/generic/uxn.team.png
